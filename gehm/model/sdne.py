...
class tSDNEmodel(torch.nn.Module):
    def __init__(self, dim_input:int, dim_intermediate:int, dim_embedding:int, activation:torch.nn.Module=torch.nn.Tanh,
        ...
        # Init layers
        LayerList=[]
        for i in range(0,nr_encoders+1):
            LayerList.append(torch.nn.TransformerEncoderLayer(dim_input, nr_heads, dim_feedforward=dim_intermediate, dropout=dropout, activation=encoder_activation))
        ...
        for i in range(0,nr_decoders+1):
            LayerList2.append(torch.nn.TransformerEncoderLayer(dim_input, nr_heads, dim_feedforward=dim_intermediate, dropout=dropout, activation=encoder_activation))
        LayerList2.append(torch.nn.Linear(self.dim_input,self.dim_input))
        LayerList2.append(activation())
        # Build sequential network
        self.decoder=torch.nn.Sequential(*LayerList2)
