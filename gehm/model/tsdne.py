...
class tSDNEmodel(torch.nn.Module):
    ...
    def forward(self, batch:torch.Tensor)->Tuple[torch.Tensor,torch.Tensor]:
        ...
        if len(batch.shape) > 2: #received a batched sample
            batch=batch.permute(1, 0, 2) # Transformer expects Seq_Length-Batch-Embedding
        else:
            batch=batch.unsqueeze(1)
        
        total_nodes=batch.shape[0]*batch.shape[1]
        output=self.encoder(batch) # SxBxE
        output=output.permute(1,0,2) # BxSxE
        # Reduce to focal nodes
        output=output[:,0,:]
